...
class telnetReversi {
	...
	public static Scanner server_scanner = new Scanner(System.in);
	...
	public static void printBoard() {
		if (displayOn) {
			...
			os.println("   a   b   c   d   e   f   g   h");
			os.println(" +---+---+---+---+---+---+---+---+");
			for(int i=0; i<8; i++) {
				os.print((i+1) + "| ");
				os.print(board[0][i][currentMove]);
				os.print(" | ");
				os.print(board[1][i][currentMove]);
				os.print(" | ");
				os.print(board[2][i][currentMove]);
				os.print(" | ");
				os.print(board[3][i][currentMove]);
				os.print(" | ");
				os.print(board[4][i][currentMove]);
				os.print(" | ");
				os.print(board[5][i][currentMove]);
				os.print(" | ");
				os.print(board[6][i][currentMove]);
				os.print(" | ");
				os.print(board[7][i][currentMove]);
				if (i == 2) {
					os.println(" |  SCORE");
				} else if (i == 3) {
					os.println(" | White: " + whiteCount);
				} else if (i == 4) {
					os.println(" | Black: " + blackCount);
				} else if (i == 6) {
					os.println(" | " + messageSide1);
				} else if (i == 7) {
					os.println(" | " + messageSide2);
				} else {
					os.println(" |");
				}
				os.println(" +---+---+---+---+---+---+---+---+");
	...
	public static void printMenu() {
		...
		os.println("                     REVERSI!"								);
		os.println("=========================================================="	);
		os.println(" *Enter \"quit\" to quit."									);
		os.println(" *Enter \"display_off\" to turn off the display."			);
		os.println(" *Enter \"display_on\" to turn on the display."				);
		os.println(" *Enter \"undo\" to undo your previous move."				);
		os.println(" *Enter \"redo\" to redo your previous move."				);
		os.println(" *Enter \"menu\" to show this menu."						);
		os.println(" *Enter the column and row (ex: 'd3')  to place a piece."	);
	}
	
	public static void copyState(){
		for(int i=0; i<8; i++) {
			for(int j=0; j<8; j++){
				if (board[i][j][currentMove] != Piece.EMPTY) {
	...
	public static void display(boolean on) {
		if (displayOn) {
			...
			if (on) {
				...
				os.println("The display is already OFF");
			}
		}
	}
	...
	public static void gameStart() {
		...
		os.println(" Select game mode:");
		os.println("+-----------------+");
		os.println(" 1. 2 player local");
		os.println(" 2. 1 player easy");
		os.println(" 3. 1 player medium");
		os.println(" 4. 1 player hard");
		os.println(" 5. 1 player (Random)");
		
		// get input for game selection
		while (!validGameSelection) {
			try {
				os.print("> ");
				gameSelection = is.readLine();
				switch (Integer.parseInt(gameSelection)) {
					case 1:
					...
					case 5:
						...
						break;
					default:
						os.println("Invalid game choice");
						break;
				}
			} catch (Exception e) {
				os.println("You didnt enter an integer, did you?");
				scanner.next();
			}
		}
		os.print("Player 1, select you're color (W/w - white, B/b - black): ");
		String colorSelection = "";
		while (true) {
			try {
				colorSelection = is.readLine();
			} catch (IOException e) {
				os.println("ERROR: ");
			}
			if (colorSelection.toLowerCase().equals("w")) {
				player1 = Piece.WHITE;
				player2 = Piece.BLACK;
				os.println("Player 1 is white.");
				player1Turn = false;
				break;
			} else if (colorSelection.toLowerCase().equals("b")) {
				player1 = Piece.BLACK;
				player2 = Piece.WHITE;
				os.println("Player 1 is black.");
				player1Turn = true;
				break;
			} else {
				os.print("Invalid Choice. Try again: ");
			}
		}
		os.println(gameStarting);
		printMenu();
		playGame(Integer.parseInt(gameSelection));
	}
